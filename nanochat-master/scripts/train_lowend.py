"""
Train model (Low-End GPU Variant)
Optimized for RTX 3050 6GB or similar.
Supports 8-bit Optimizer and Float16 Mixed Precision.
"""
...
from nanochat.gptlowend import GPT, GPTConfig, Block # Use GPT LowEnd
...
# Model architecture
depth = 12 # Default depth for low-end
max_seq_len = 1024 # Default seq len for low-end
# Training horizon. Only one of these 3 will be used, in this order of precedence.
num_iterations = -1 # explicit number of steps of the optimization (-1 = disable)
target_flops = -1.0 # calculate num_iterations to reach target_flops. Useful for scaling laws experiments (-1 = disable)
target_param_data_ratio = 20 # calculate num_iterations to maintain fixed data:param ratio (Chinchilla=20) (-1 = disable)
# Optimization
device_batch_size = 2 # Low default
total_batch_size = 65536 # Smaller total batch size
...
eval_tokens = 20*1024 # number of tokens to evaluate val loss on
core_metric_every = 2000 # every how many steps to evaluate the core metric (-1 = disable)
core_metric_max_per_task = 100 # fewer examples per task
sample_every = 2000 # every how many steps to sample from the model
checkpoint_every = 50 # every how many steps to write a checkpoint (<=0 = disable)
# Output
model_tag = "" # optionally override the model tag for the output checkpoint directory name
use_8bit_optimizer = True # Enable 8-bit optimizer by default
...
# FORCE FLOAT16 (Half Precision) as requested
autocast_ctx = torch.amp.autocast(device_type=device_type, dtype=torch.float16) if device_type in ("cuda", "xpu") else nullcontext()
...
if use_fsdp:
    ...
    # Block is imported from gptlowend
...
if use_fsdp:
    ...
    print0("FSDP initialized (float16).")
...
if use_fsdp:
    # With FSDP, we fallback to a single AdamW optimizer for simplicity and stability for now.
    print0("Using FSDP: Fallback to AdamW optimizer (skipping Muon).")
    # TODO: Check if bitsandbytes works with FSDP, likely tricky, sticking to torch.optim.AdamW
    # But user wanted 8bit.
    # If safe_8bit is desired with FSDP, we might need more work. 
    # For now, let's assume FSDP + 8bit might be unstable or unsupported in this simple script.
    # We will use standard AdamW for FSDP fallback, or try 8bit if user forced it.
    AdamWClass = torch.optim.AdamW
    if use_8bit_optimizer:
        try:
             import bitsandbytes as bnb
             AdamWClass = bnb.optim.AdamW8bit
             print0("Attempting to use bitsandbytes 8-bit AdamW with FSDP (Experimental).")
        except ImportError:
             pass
    
    optimizers = [AdamWClass(model.parameters(), lr=0.0006, betas=(0.9, 0.95), weight_decay=0.1)]
    adamw_optimizer = optimizers[0]
    muon_optimizer = None 
else:
    # Pass use_8bit_optimizer flag to setup_optimizers
    optimizers = model.setup_optimizers(unembedding_lr=unembedding_lr, embedding_lr=embedding_lr, matrix_lr=matrix_lr, weight_decay=weight_decay, use_8bit_optimizer=use_8bit_optimizer)
...
def save_base_checkpoint(step_to_save):
    ...
    # For FSDP, we need to get the full state dict via FSDP context
    if use_fsdp:
...
for step in range(start_step, num_iterations + 1):
    ...
    if not use_fsdp and muon_optimizer:
